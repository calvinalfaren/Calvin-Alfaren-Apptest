...
const IndexHomeContainer = () => {
  ...
  const renderContact = ({ item }) => {
    ...
    return (
      <TouchableOpacity 
        // onPress={() => navigation.navigate('detailContact', {contactId})}
        onPress={()=> redirect(contactId)}
        style={{backgroundColor: '#e5e5e5', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, marginBottom: 10}}>
  ...
  const redirect = (contactId) => {
    ...
    dispatch(DetailContact.action({contactId}))
    return navigation.navigate('detailContact', {contactId})
  };
